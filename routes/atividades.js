...
module.exports = (app) => {
    ...
    app.get("/feito", async (req, res) => {
        //recuperar o parametro id da barra de endereço
        var id = req.query.id
        var feito = await atividades.findOneAndUpdate(
            { _id: id },
            { status: 2 }
        )
        //redirecionar para a rota atividades
        res.redirect('/atividades?id=' + feito.usuario)
    })

    app.get("/cancelado", async (req, res) => {
        //recuperar o parametro id da barra de endereço
        var id = req.query.id
        var cancelado = await atividades.findOneAndUpdate(
            { _id: id },
            { status: 1 }
        )
        //redirecionar para a rota atividades
        res.redirect('/atividades?id=' + cancelado.usuario)
    })
